...
遷移是一種方便的管理模式，它讓你可以有條不紊地改變你地資料庫。你當然可以選擇手0動編輯 SQL 片段，然後告訴開發人員們要記得執行這些片段。但在下一次佈署專案時，說不定你還得追蹤在正式伺服器上是否執行了這些片段。

而資料庫資料表 **遷移** 功能會自動追蹤哪些資料遷移已經成功執行過了，你只要確定你的遷移已經設定完畢，然後呼叫 ``$migration->latest()`` 將資料庫更新到最新的狀態。你也可以使用 ``$migration->setNamespace(null)->progess()`` 來包含所有命名空間中的遷移項目。
...
每個遷移都是按照數字順序向前或向後執行，每個遷移都會使用創建遷移時的時間戳進行編號，格式為 **YYYYMMDDHHIISS** （例如： **20121031100537** ）。這有助於防止在團隊開發中發生編號衝突。
...
當你的資料表具有外來鍵的特性時，在試圖刪除資料表或資料列等動作，遷移經常會出現問題。要在執行遷移時暫時繞過外來鍵的檢查，請在資料庫連接上使用 ``disableForeignKeyChecks()`` 以及 ``enableForeignKeyChecks()`` 方法。
...
遷移將只針對一個資料庫群組執行，如果你在 **app/Config/Database.php** 中定義了多個群組，那麼它將預設在組態設定檔案中指定的 ``$defaultGroup`` 上執行。或許有時，你需要不同資料庫的不同模式，你擁有一個資料庫用於記錄一般的站點訊息，而另一個資料庫用於任務關鍵資料的存取，你可以透過在遷移時設定 ``$DBGroup`` 來確保遷移只針對正確的資料庫群組執行，當然，這個名稱必須與資料庫群組的名稱完全一致。
...
當使用 ``-all`` 選項，它將掃描所有的命名空間，試圖找到任何尚未執行過的遷移。這些遷移都將被記錄起來，再以創建日期進行排序後執行。這將有助於幫助任何應用程式與模組之間的淺在衝突。
...
.. php:class:: CodeIgniter\\Database\\MigrationRunner
    ...
    .. php:method:: latest($group)
        ...
        這將在一個命名空間（或所有的命名空間）中定位遷移，確定那些遷移還沒有被執行過，並按照它們的版本順序執行（不管位於哪個命名空間都將參與排序）。
