...
強化測試的常見方法是使用特性來合併不同測試案例的過渡環境。　``CIUnitTestCase`` 將檢查所有類別特徵，並訓找以特徵命名的過渡方法。例如：如果您需要向某些測試案例加入身份認證，則可以使用 ``set up`` 方法來建立身份驗證特徵以偽裝使用者登入：
...
與服務相似，您可能會發現自己需要在測試期間提供一個預先組態的類別實體，這個實體將與 ``Factories`` 一起使用。你可以使用相同的 ``injectMock()`` 和 ``reset()`` 靜態方法（就像 **Services**），但它們需要一個額外的元件名稱做為前置參數：
